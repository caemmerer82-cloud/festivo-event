...
#  Usage: sudo bash /opt/festivo-event/deploy/update.sh
# =============================================================================
set -euo pipefail
...
mysql -u root festivo_event 2>/dev/null <<'SQL' || true
...
for f in "$INSTALL_DIR/database/migrations/"*.sql; do
    name="$(basename "$f")"
    exists=$(mysql -u root -sN festivo_event -e "SELECT COUNT(*) FROM _migrations WHERE name='$name'" 2>/dev/null || echo 0)
    if [[ "$exists" == "0" ]]; then
        info "Applying migration $name…"
        mysql -u root festivo_event < "$f"
        mysql -u root festivo_event -e "INSERT INTO _migrations (name) VALUES ('$name')"
    fi
done
